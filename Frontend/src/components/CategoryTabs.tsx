    ...
    <div className="relative w-full overflow-hidden rounded-2xl border border-white/70 bg-gradient-to-br from-white via-white to-rose-50 shadow-[0_16px_40px_rgba(231,74,59,0.12)] px-4 py-4">
      ...
      <div className="relative z-10">
        ...
        <button
          ...
          disabled={!canGoPrev}
          className={`hidden md:block absolute left-2 top-1/2 transform -translate-y-1/2 z-20 rounded-full p-2.5 transition-all duration-200 border border-white/70 backdrop-blur-sm shadow-sm ${canGoPrev ? "bg-white/90 hover:bg-white text-gray-600 hover:text-gray-800" : "opacity-0 pointer-events-none"
            }`}
        ...
        <div className="px-2 md:px-6">
              ...
              {categories.map((category) => {
                ...
                return (
                  <button
                    ...
                    className={`flex flex-col items-center py-3 px-4 transition-all duration-200 relative flex-shrink-0 rounded-xl overflow-hidden ${isActive ? 'transform scale-105 bg-gradient-to-br from-red-50 to-orange-50 shadow-lg' : 'hover:scale-102 hover:bg-gray-50'
                      }`}
                  ...
                  >
                    ...
                    <div className="w-12 h-12 flex items-center justify-center mb-2 relative z-10">
                      <div className={`w-10 h-10 transition-all duration-300 ${isActive ? 'text-red-500 drop-shadow-[0_0_8px_rgba(239,68,68,0.5)]' : 'text-gray-500'}`}>
                        {category.icon}
                      </div>
                    </div>
                    ...
                    <div className="w-full flex justify-center px-1 relative z-10">
                      <span className={`text-[10px] font-medium text-center leading-tight transition-colors duration-200 max-w-full ${isActive ? 'text-gray-900 font-semibold' : 'text-gray-600 hover:text-gray-800'
          ...
          {/* Desktop: Original 6-column Grid with Pagination */}
            ...
            {visibleCategories.map((category) => {
              ...
              return (
                <button
                  ...
                  onClick={(e) => handleCategoryClick(category.id, e)}
                  className={`flex flex-col items-center py-2 px-2 transition-all duration-200 relative w-full min-w-0 rounded-xl overflow-hidden ${isActive ? 'transform scale-105 bg-gradient-to-br from-red-50 to-orange-50 shadow-lg' : 'hover:scale-102 hover:bg-gray-50'
                    }`}
                ...
                >
                  ...
                  {/* Category Name */}
                  <div className="w-full flex justify-center px-1 relative z-10">
                    <span className={`text-xs font-medium text-center leading-tight transition-colors duration-200 whitespace-nowrap overflow-hidden text-ellipsis max-w-full ${isActive ? 'text-gray-900 font-semibold' : 'text-gray-600 hover:text-gray-800'
        ...
        <button
          onClick={goToNext}
          disabled={!canGoNext}
          className={`hidden md:block absolute right-2 top-1/2 transform -translate-y-1/2 z-20 rounded-full p-2.5 transition-all duration-200 border border-white/70 backdrop-blur-sm shadow-sm ${canGoNext ? "bg-white/90 hover:bg-white text-gray-600 hover:text-gray-800" : "opacity-0 pointer-events-none"
            }`}
